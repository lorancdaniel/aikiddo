...
For production text stages (`lyrics.generate` and `characters.import_or_approve`), uncomment `OPENAI_API_KEY` in `.env.ops` and put the real key there before starting the backend. The backend passes only the allowlisted provider variables (`OPENAI_API_KEY`, `AIKIDDO_OPENAI_TEXT_MODEL`, `AIKIDDO_OPENAI_TIMEOUT_SEC`, `AIKIDDO_WORKER_MODE`) into the SSH worker command. Without `OPENAI_API_KEY`, production text generation fails closed and does not write a success manifest.
...
- Current provider path: lyrics.generate and characters.import_or_approve use OpenAI Responses API when OPENAI_API_KEY is available; deterministic worker mode is dev-only.
